...
if (!artist.equals("") && !artist.equals("전체") && !search.equals("")) { // 아티스트별 검색
	...
	pageInfo = Commons.getPageInfo(dao.getCountSearch(pageNumber, category, artist, search), pageNumber,
	commentPerPage);
...
<body class="admin_artist_comment">
	...
	<section class="container text-center">
		...
		<form name="admin_artist_comment_search_form" id="admin_artist_comment_search_form" action="" method="get">
			...
			<div class="d1 container text-right">
				<div class="box">
					<label>가수</label>
					<select class="custom-select" name="artist" id="artist_select">
						<%
		...
		<table class="table">
			...
			<tr class="row">
				...
				<td class="comment_buttons col-md-3">
					<div class="input-group">
						<span class="s1">신고수 <%=vo.getReport()%></span> <span class="s1">추천수 <%=vo.getRecommend()%></span>
		...
		<ul class="pagination justify-content-center mt-3">
			...
			<li class="page-item <%if (pageNumber == 1) {%> disabled <%}%>">
				<a class="page-link" href="<%=Commons.get_page(url, inputs, 1)%>">&laquo;</a>
			</li>
			<li class="page-item <%if (pageInfo.get("prev") != 1) {%> disabled <%}%>">
				<a class="page-link" href="<%=Commons.get_page(url, inputs, pageNumber - 1)%>">&lt;</a>
			</li>
			<%
			...
			<li class="page-item <%if (i == pageNumber) {%>active <%}%>">
				<a class="page-link" href="<%=Commons.get_page(url, inputs, i)%>"><%=i%></a>
			</li>
			<%
				}
			%>
			<!-- 현 페이지가 마지막 페이지일 경우 다음 페이지 비홯성화 -->
			<li class="page-item <%if (pageInfo.get("next") != 1) {%> disabled <%}%>">
				<a class="page-link" href="<%=Commons.get_page(url, inputs, pageNumber + 1)%>">&gt;</a>
			</li>
			<li class="page-item <%if (pageNumber == end) {%> disabled <%}%>">
				<a class="page-link" href="<%=Commons.get_page(url, inputs, pageInfo.get("totalPage"))%>">&raquo;</a>
			</li>
		</ul>
	</section>
	<!-- 삭제 메시지 팝업 -->
